...
static disp_val try_recv(disp_val ch, disp_val body, int *executed) {
    ...
    while (NN(body_it) && T(body_it) == FLAG_CONS) {
        result = disp_eval(disp_global_env, disp_car(body_it));
        body_it = disp_cdr(body_it);
    }

    if (NN(IT))
        disp_define_symbol(disp_global_env, it_sym, old_it, 0);
    else
        disp_define_symbol(disp_global_env, it_sym, NIL, 0);
...
static disp_val select_builtin(disp_env_t *env, disp_val expr) {
    ...
    for (disp_val c = clauses; NN(c) && T(c) == FLAG_CONS; c = disp_cdr(c), i++) {
        ...
        if (op_id == SYM_ID(RECV)) {
            ...
            disp_val ch_arg = disp_eval(disp_global_env, ch_expr);
            ...
            disp_val val_arg = disp_eval(disp_global_env, val_expr);
            ...
            disp_val ms_val = disp_eval(disp_global_env, ms_expr);
            long ms = 0;
            ...
            gc_free(infos);
            ERET(NIL, "select: unknown operation %s", SYM_NAME(op_name));
        }
    }
