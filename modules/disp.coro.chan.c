...
disp_channel_t* disp_get_channel(disp_box v) {
    return v->data->chan;
}

void disp_set_channel(disp_box v, disp_channel_t *c) {
...
static disp_box make_channel_syscall(disp_box *args, int count) {
    ...
    ch->head = 0;
    ch->tail = 0;
    ch->buffer = (cap > 0) ? gc_typed_calloc(cap, sizeof(disp_box), &GC_TYPE_PTR_ARRAY) : NULL;
...
static disp_box channel_recv_syscall(disp_box *args, int count) {
    ...
    while (1) {
        ...
        if (ch->cap == 0) {
            // 无缓冲通道：直接与等待的发送者握手
            if (ch->wait_send) {
                disp_box waiter = ch->wait_send;
                ch->wait_send = waiter->data->coro->next;
                disp_box value = ch->direct_value;
                ch->direct_value = NIL;
                scheduler_add(waiter);
                gc_pthread_mutex_unlock(ch->lock);
                return value;
            ...
            // 有缓冲通道
            if (ch->size > 0) {
                disp_box value = ch->buffer[ch->head];
