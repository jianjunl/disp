...
static char* val_to_write_str(disp_val v) {
    if (E(v, NIL)) return gc_strdup("nil");
    if (E(v, TRUE)) return gc_strdup("true");
...
static disp_val make_string_syscall(disp_val *args, int count) {
    ...
    memset(buf, fill, len);
    buf[len] = '\0';
    disp_val result = disp_make_str(buf);
    gc_free(buf);
    return result;
}
...
static disp_val string_append_syscall(disp_val *args, int count) {
    size_t total_len = 0;
    const char **parts = gc_typed_malloc(count * sizeof(char*), &GC_TYPE_PTR_ARRAY);
    if (!parts) return NIL;
    for (int i = 0; i < count; i++) {
...
static disp_val list_to_string_syscall(disp_val *args, int count) {
    ...
    // 计算长度并收集字符
    size_t len = 0;
    for (disp_val p = args[0]; NN(p) && T(p) == FLAG_CONS; p = disp_cdr(p)) {
...
static disp_val string_copy_syscall(disp_val *args, int count) {
    ...
    return disp_make_str(disp_get_str(args[0]));
}
...
static disp_val number_to_string_syscall(disp_val *args, int count) {
    ...
    if (!s) return NIL;
    disp_val result = disp_make_str(s);
    gc_free(s);
    return result;
}
